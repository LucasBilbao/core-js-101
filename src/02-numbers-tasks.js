...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const distance = (((x2 - x1) ** 2) + ((y2 - y1) ** 2)) ** (1 / 2);
  return distance;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const distanceMult = getDistanceBetweenPoints(0, 0, x1, y1)
  * getDistanceBetweenPoints(0, 0, x2, y2);
  return Math.acos(scalar / distanceMult);
}
...
function roundToPowerOfTen(num, pow) {
  const rounded = Math.round(num / (10 ** pow)) * (10 ** pow);
  return rounded;
}
